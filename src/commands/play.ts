...
export const command: Command = {
    ...
    execute: (client: Client, msg: Message, args: Array<any>): Promise<void> => {
        ...
        if (msg.guild.me.voice.connection) {
            if (args.length === 1) {
                try {
                    msg.guild.me.voice.connection.play(args[0]).on("finish", () => {
                        msg.reply("Your music finished");
                    });
                ...
                msg.guild.me.voice.connection.play("https://upload.wikimedia.org/wikipedia/commons/e/e5/Tetris_theme.ogg").on("finish", () => {
                    msg.reply("Want to listen to more Tetris?");
                });
                embed.setColor("#0000FF");
                embed.setTitle(">:)");
